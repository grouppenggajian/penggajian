...
//        return $sqlsearch;
        $orderby='';
        $dataall=MYModel::SP_getData('sp_gaji_potongan',array('countdata',$thbl,$sqlsearch,$orderby,''));
        $vlimit="limit $start,$limit";
        
            
        $data = MYModel::SP_getData('sp_gaji_potongan',array('loaddata',$thbl,$sqlsearch,$orderby,$vlimit));
        return json_encode([
                    'success' => true,
                    'data' => $data,
    ...
    public function executeRow(Request $request){
        ...
        $data=MYModel::SP_execData('sp_calc_potongan',array($thbl,$query[0]->tglawal,$query[0]->tglakhir),true);                

...
Ext.define("Ext.ux.exporter.excelFormatter.Worksheet", {
  ...
  constructor: function(store, config) {
    ...
    Ext.applyIf(config, {
      ...
      title  : "wroksheet",
//      title  : "��񵼳�",
      columns    : store.getModel().getFields() == undefined ? {} : store.getModel().getFields().items
    });
    ...
    var insertRow=function(col,record,me){
        ...
        if(name) {
            if (Ext.isFunction(col.renderer)) {
                ...
                var value = record.get(name), type  = me.typeMappings[col.type || record.store.getModel().getField(name).type];
